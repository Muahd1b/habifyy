...
- Showcase current level, point balance, and recent alert history.
...
4. Inspect level progress, upcoming bonuses, and domain impact summaries.
5. Edit profile information or sign out.
...
- Communicating level status and cross-domain habit influence without exposing private notes.

## Metrics
- Profile views per active user.
- Follow/follower growth.
- Social link engagement (click-through, if tracked).
- Level progression visibility and alert acknowledgment rate.

## Backend Alignment
- **Tables**: `profiles`, `streak_history`, `followers`, `social_links`, `user_records`, `notification_channels` enable the profile surface; ensure follower counts are updated via triggers when friendships/follows mutate and include point/level tallies.
- **Views**: Pull summary cards from `profile_stats_daily` and expose RPC `profiles.get_overview(target_user_id, viewer_id)` to enforce privacy and block rules, including domain impact narratives.
